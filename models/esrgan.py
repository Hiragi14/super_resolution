...
class DiscriminatorVGG128(nn.Module):
    """Discriminator for ESRGAN (input size: 128x128)"""
    def __init__(self, in_channels=3, base_channels=64):
        super(DiscriminatorVGG128, self).__init__()

        def conv_block(in_nc, out_nc, stride):
            return nn.Sequential(
                nn.Conv2d(in_nc, out_nc, kernel_size=3, stride=stride, padding=1),
        ...
        self.classifier = nn.Sequential(
            nn.Linear(base_channels * 8 * 8 * 8, 100),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(100, 1)
        )

    def forward(self, x):
        feat = self.features(x)  # shape: [B, C, 8, 8]
